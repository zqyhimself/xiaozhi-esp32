#include "common/dual_network_board.h" // Changed from ml307_board.h
...
#include "common/power_save_timer.h"
#include "esp32_camera.h" // For Esp32Camera
#include <esp_lcd_touch_ft5x06.h> // For FT5x06 touch
#include <esp_lvgl_port.h> // For LVGL port/touch integration
...
#include <esp_timer.h>
#include <wifi_station.h> // Added for WifiStation::GetInstance().IsConnected()
...
public:
    LichuangDevPlusAudioCodec(i2c_master_bus_handle_t i2c_bus, Pca9557* pca9557)
        : BoxAudioCodec(i2c_bus,
                       ...
                       AUDIO_I2S_GPIO_DOUT,
                       AUDIO_I2S_GPIO_DIN,
                       GPIO_NUM_NC, // PA Enable Pin (not used if PCA9557 controls amp)
...
private:
    ...
    void InitializeCamera() {
        // Open camera power
        if (pca9557_) pca9557_->SetOutputState(2, 0); // PCA9557 IO2 set to 0 to power camera

        camera_config_t config = {};
        config.ledc_channel = LEDC_CHANNEL_2; // Using different LEDC from PWM Backlight (LEDC_TIMER_0, LEDC_CHANNEL_0)
        config.ledc_timer = LEDC_TIMER_2;   // if Backlight uses LEDC_TIMER_0, CHANNEL_0
        ...
        config.pin_sccb_sda = CAMERA_PIN_SIOD; // Using pins defined in config.h
        config.pin_sccb_scl = CAMERA_PIN_SIOC;
        config.sccb_i2c_port = (i2c_port_t)1;   // Use the same I2C port as other devices (port 1 for this board)
        config.pin_pwdn = CAMERA_PIN_PWDN;
        config.pin_reset = CAMERA_PIN_RESET;
        config.xclk_freq_hz = XCLK_FREQ_HZ;
        config.pixel_format = PIXFORMAT_RGB565; // Changed from JPEG for faster processing if needed, or use PIXFORMAT_JPEG
        config.frame_size = FRAMESIZE_VGA;      // VGA (640x480), or use smaller like QVGA for speed
        config.jpeg_quality = 12; // 0-63, lower number means higher quality (ignored if not JPEG)
    ...
    void InitializeTouch() {
        if (!i2c_bus_ || !lv_display_get_default()) {
             ESP_LOGE(TAG, "I2C bus or LVGL display not initialized before Touch");
             return;
        }
        ...
        esp_lcd_touch_config_t tp_cfg = {
            ...
            .rst_gpio_num = GPIO_NUM_NC, // Assuming no reset pin for FT5x06 or not used
            .int_gpio_num = GPIO_NUM_NC, // Assuming no interrupt pin or not used
            .levels = {
                .reset = 0, // Default level
                .interrupt = 0, // Default level
            },
            .flags = {
                .swap_xy = 1, // As per original code
                .mirror_x = 1,
                .mirror_y = 0,
            },
        };
        ...
        lv_disp_t* lv_disp = GetDisplay() ? (GetDisplay()->GetLvglPort() ? GetDisplay()->GetLvglPort()->get_disp_handle() : nullptr) : nullptr;
        if (!lv_disp) {
            ESP_LOGE(TAG, "LVGL display handle not available for touch input.");
            // Consider deleting tp_io_handle if it was successfully created before this error
            if (tp_io_handle) {
                esp_lcd_panel_io_del(tp_io_handle);
            }
            // Consider deleting tp handle if created
            if (tp) {
    ...
    void InitializeButtons() {
        ...
        boot_button_.OnLongPress([this]() {
            if (power_save_timer_) power_save_timer_->WakeUp();
            ESP_LOGI(TAG, "Boot button long press detected - AXP2101 should handle power off if configured.");
        });
...
public:
    LichuangDevPlusBoard() : DualNetworkBoard(ML307_TX_PIN, ML307_RX_PIN, ML307_RX_BUFFER_SIZE), boot_button_(BOOT_BUTTON_GPIO), pca9557_(nullptr), pmic_(nullptr), volume_up_button_(nullptr), volume_down_button_(nullptr), display_(nullptr), headphone_check_timer_(nullptr), power_save_timer_(nullptr), camera_(nullptr) {
        InitializeI2c();
        InitializeSpi();
        InitializeSt7789Display(); // Display must be first for LVGL port if touch uses it
        InitializeTouch();         // Touch after Display and I2C
        InitializePowerSaveTimer();
        InitializeButtons();
        InitializeCamera();        // Camera after I2C/PCA9557
        InitializeIot();
        GetBacklight()->RestoreBrightness();

        // Initial headphone check and speaker state set
        check_headphone_status();

        const esp_timer_create_args_t headphone_timer_args = { // Keep existing headphone timer
